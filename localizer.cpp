...
/**
    ...
    @return - a normalized two dimensional grid of floats. For 
           a 2x2 grid, for example, theis would be:
...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) 
{
	...
	{
		row.clear(); // emptying the row each time after appended to beliefs -- idea taken from premade zeros function 
...
vector< vector <float> > sense(char color, 
	...
	float p_miss) 
{
	// Done to create a 2d vector of the same dimensions
	// could have also created a 2d vector of 0s with the same dimensions 
	vector< vector <float> > newBeliefs = beliefs; 

	// Looping through the grid that defines the colors 
    // in order to calculate new probabilities (based on p_hit/miss)
	for (int i = 0; i < grid.size(); i++)
	{
		for (int j = 0; j < grid[0].size(); j++)
		{
			if (grid[i][j] != color)
			{
				newBeliefs[i][j] = (beliefs[i][j] * p_miss);
			}
			else
			{
				newBeliefs[i][j] = (beliefs[i][j] * p_hit);
			}
		}
	}
	...
	float blurring) 
{
	...
	vector < vector <float> > newGrid = zeros(height, width); // using the zeros function in helpers.cpp
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			int new_j = (i + dx) % height;
			newGrid[new_i][new_j] = beliefs[i][j]; // beliefs[i][j] is the 'cell' 
		}
	}
